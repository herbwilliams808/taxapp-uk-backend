...
namespace UnitTests.Application.Calculators
{
    public class BasicRateLimitCalculatorTests
    {
            ...
            decimal expectedLimit)
        {
            ...
            // Act
            var result = new BasicRateLimitCalculator().Calculate(basicRateThreshold, pensionContributions, giftAidContributions);

            // Assert
            Assert.Equal(expectedLimit, result.BasicRateLimitValue);
        }
        ...
        public void Calculate_WithNegativeContributions_ThrowsArgumentException()
        {
            ...
            Assert.Throws<ArgumentException>(() =>
                new BasicRateLimitCalculator().Calculate(basicRateThreshold, pensionContributions, giftAidContributions));
        }
    }
}